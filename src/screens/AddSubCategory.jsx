  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/staff/addSubCategory`,
        {
      ...
      if (response.data.status === 'SUCCESS') {
        ...
        setFormData({ categoryId: '', subCategoryName: '', description: '' })
        setTimeout(() => {
      ...
      if (err.response) {
        const message = err.response.data?.message || 'Failed to add subcategory'
            ...
            >
              ...
              {categories.map((category) => (
                <option key={category._id} value={category._id} className="bg-black/80">
                  {category.categoryName} {category.isActive ? '' : '(Inactive)'}
                </option>
              ))}
            </select>
            {loadingCategories && (
