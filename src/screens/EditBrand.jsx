    ...
    const fetchBrand = async () => {
      ...
      try {
        ...
        if (response.data.status === 'SUCCESS') {
          const brand = response.data.data.find(b => b._id === id)
          if (brand) {
            setFormData({
        ...
        setError(err.response?.data?.message || 'Error fetching brand')
      } finally {
        setFetching(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_API_URL}/staff/editBrand/${id}`,
        {
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-3 text-[#998f82]">Loading brand...</span>
      </div>
    )
  }
